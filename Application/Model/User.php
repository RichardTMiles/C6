...
class User extends GlobalMap
{
    ...
    public function login($username, $password, $rememberMe)
    {
        ...
        $sql = 'SELECT user_password, user_first_name, user_last_name, user_profile_pic, user_id FROM user WHERE user_username = ?';
    ...
    public function facebook($request)
    {
        ...
        if (!$C6_id && !$fb_id): // create new account
            if ($request === 'SignUp'):                         // This will set the session id
                ...
                return $request = 'SignUp';        // Sign into a non-existing account
            endif;
        elseif ($C6_id && !$fb_id):
            if ($request === 'SignIn'):
                $sql = "UPDATE StatsCoach.user SET user_facebook_id = ? WHERE user_id = ?";     // UPDATE user
